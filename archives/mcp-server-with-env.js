#!/usr/bin/env node
/**
 * Claudette MCP Server with Environment Variable Loading
 * Fixed version that properly loads .env file
 */
...
function loadEnvironment() {
  ...
  try {
    ...
    console.error('[MCP] Environment variables loaded from .env');
    return true;
  } catch (error) {
...
class ClaudetteMCPServer {
  ...
  async setupKeychain() {
    try {
      ...
      this.apiKeys = {
        OPENAI_API_KEY: openaiKey.stdout.trim() || process.env.OPENAI_API_KEY,
        CODELLM_API_KEY: codellmKey.stdout.trim() || process.env.CODELLM_API_KEY,
      ...
      this.apiKeys = Object.fromEntries(
        Object.entries(this.apiKeys).filter(([key, value]) => value && value.trim())
      );
      
      console.error(`[MCP] API keys loaded: ${Object.keys(this.apiKeys).join(', ')}`);
    } catch (error) {
      console.error('[MCP] Warning: Could not load API keys:', error.message);
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      child.on('close', (code) => {
        if (code === 0) {
          ...
          // Try to extract meaningful information even on failure
          const meaningfulOutput = errorOutput.split('\n').filter(line => 
          ...
          if (meaningfulOutput.trim()) {
            resolve(`Status (partial): ${meaningfulOutput}\n\nFull error output: ${errorOutput}`);
          } else {
  ...
  start() {
    console.error('[MCP] Claudette MCP Server with Environment Loading starting...');
    ...
    console.error('[MCP] Claudette MCP Server ready with environment loaded');
  }
}
