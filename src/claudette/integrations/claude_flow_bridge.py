...
class ClaudeFlowBridge:
    ...
    def _check_claude_flow_availability(self) -> bool:
        ...
        try:
            # Set a very short timeout for initial check
            ...
            # For integration purposes, mark as available if npx exists
            # Actual functionality will be tested when used
            try:
                subprocess.run(["npx", "--version"], capture_output=True, timeout=2)
                # If npx works, assume claude-flow can be installed/used
                self.claude_flow_version = "available (not detected)"
                return True
            except:
                return False
